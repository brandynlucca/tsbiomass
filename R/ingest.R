...
fetch_fishbase <- function(species, cache_path = NULL, refresh = FALSE) {
  ...
  source_aliases_by_target <- split(names(source_aliases), unname(source_aliases))
  ...
  parse_fishbase_fao_basin <- function(x) {
    vapply(
      as.character(x),
      function(one) {
  ...
  for (endpoint_name in names(endpoints)) {
    ...
    for (source_col in source_cols) {
      if (identical(endpoint_name, "faoareas") && identical(source_col, "fao")) {
        value_df <- dat[, c("species_name_query", source_col), drop = FALSE]
        names(value_df)[2] <- "value_raw"
        value_df$value_raw <- parse_fishbase_fao_basin(value_df$value_raw)
        value_df <- value_df |>
          dplyr::filter(!is.na(.data$value_raw)) |>
          dplyr::group_by(.data$species_name_query) |>
          dplyr::summarise(
            value = paste(unique(unlist(stringr::str_split(.data$value_raw, ";"))), collapse = ";"),
            .groups = "drop"
          )
        names(value_df)[2] <- "ocean_basin"
        out <- dplyr::left_join(out, value_df, by = "species_name_query", suffix = c("", "_new"))
        missing_idx <- is.na(out$ocean_basin) | !nzchar(as.character(out$ocean_basin))
        out$ocean_basin[missing_idx] <- out$ocean_basin_new[missing_idx]
        out$ocean_basin_new <- NULL
        next
      }
      ...
      if (identical(endpoint_name, "faoareas") &&
        identical(target_col, "fao_area") &&
        !identical(source_col, "area_code")) {
        next
      }
      ...
      if (identical(target_col, "fao_area")) {
        value_df <- value_df |>
          dplyr::mutate(
      ...
      } else if (identical(endpoint_name, "length_weight") &&
        source_col %in% (source_aliases_by_target[["lw_a_g"]] %||% character(0))) {
        value_df <- value_df |>
          dplyr::filter(!is.na(.data$value_raw)) |>
          dplyr::group_by(.data$species_name_query) |>
          dplyr::summarise(
            value = stats::median(as.numeric(.data$value_raw), na.rm = TRUE),
            .groups = "drop"
          )
        target_col <- "lw_a_g"
      } else if (identical(endpoint_name, "length_weight") &&
        source_col %in% (source_aliases_by_target[["lw_b"]] %||% character(0))) {
        value_df <- value_df |>
          dplyr::filter(!is.na(.data$value_raw)) |>
          dplyr::group_by(.data$species_name_query) |>
          dplyr::summarise(
...
enrich_species_db <- function(db_list,
  ...
  for (db_name in names(db_list)) {
    ...
    value_cols <- setdiff(intersect(names(dat), output_cols), c("genus", "species"))
    if (nrow(dat) == 0) {
      collapsed <- tibble::as_tibble(template)[0, , drop = FALSE]
    } else {
      collapsed <- dat |>
        ...
        dplyr::summarise(
          dplyr::across(
            ...
            ~ {
              values <- .x[!is.na(.x)]
              if (length(values) == 0) template[[dplyr::cur_column()]] else values[[1]]
            }
          ),
          .groups = "drop"
        )
    ...
    missing_cols <- setdiff(output_cols, names(collapsed))
    for (nm in missing_cols) {
      collapsed[[nm]] <- template[[nm]]
    }
    collapsed_dbs[[db_name]] <- collapsed[, output_cols, drop = FALSE]
  }
  ...
  out$study_reference_id <- stringr::str_replace_all(
    stringr::str_squish(
      stringr::str_to_lower(
        dplyr::coalesce(ref_link, ref_short, paste0("row_", out$model_id))
      )
    ),
    "[^a-z0-9]+",
    "_"
