...
conjurer_refresh_reference_anchors <- function(candidates,
  ...
  join_col <- if ("model_id" %in% names(anchors_tbl) && "model_id" %in% names(models_tbl)) {
    "model_id"
...
#' Register the `Conjurer` simulate method
#'
#' @name simulate_conjurer
#' @usage NULL
#'
...
#' Coerce `Conjurer` to a tibble
#'
#' Returns the canonical trait-by-anchor summary table.
#'
#' @param x A [Conjurer] object.
#' @param ... Unused.
#'
...
#' conjurer <- as_conjurer(selector)
#' conjurer <- simulate(conjurer)
#' tibble::as_tibble(conjurer)
#' }
#'
#' @keywords internal
#' @noRd
as_tibble_conjurer <- function(x, ...) {
