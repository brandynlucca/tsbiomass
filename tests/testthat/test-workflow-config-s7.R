test_that("WorkflowConfig validates list and YAML inputs", {
  cfg_list <- minimal_workflow_config()
  tmp_dir <- tempfile("workflow-config-")
  dir.create(tmp_dir)
  ...
  expect_true(S7::S7_inherits(cfg, WorkflowConfig))
  ...
  yaml_from_ingest <- read_workflow_config(
    yaml_path,
    base_dir = tmp_dir
  )
  cfg_from_yaml <- configurer_from_yaml(yaml_path, base_dir = tmp_dir)

  expect_true(S7::S7_inherits(cfg_from_yaml, WorkflowConfig))
...
test_that("workflow config reduces cleanly to candidates config", {
  cfg <- as_configurer(
    minimal_workflow_config(),
    base_dir = tempdir()
  )

  candidate_cfg <- tsbiomass:::workflow_to_candidates_config(cfg)
...
test_that("WorkflowConfig rejects incomplete configs", {
  bad_cfg <- minimal_workflow_config()
...
test_that("workflow YAML is validated at ingestion time", {
  bad_config <- minimal_workflow_config()
  bad_config$similarity$alpha <- 2
  yaml_path <- tempfile(fileext = ".yaml")
  yaml::write_yaml(bad_config, yaml_path)

  expect_error(
    read_workflow_config(yaml_path, base_dir = tempdir()),
    "alpha"
  )
})

test_that("legacy workflow names are rejected at ingestion", {
  bad_config <- minimal_workflow_config()
...
test_that("metalearner super-methods require super_learner selection method", {
  ...
  good_config <- minimal_workflow_config()
...
test_that("uncertainty learner inherits the selection learner when omitted", {
  config_now <- minimal_workflow_config()
  ...
  cfg <- read_workflow_config(yaml_path, base_dir = tempdir())
