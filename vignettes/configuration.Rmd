...
description: >
  All valid configuration keys, their types, defaults, and what they do.
...
The `tsbiomass` pipeline is entirely configuration-driven. A single YAML or
list object — validated and normalized into a `Configurer` — establishes the
full run contract before any modeling begins. This page describes every valid
top-level section and key.
...
External YAML aliases accepted: `input` → `input_file`, `output_root` → `out_root`,
`cache_folder` → `cache_dir`, `support_folder` → `supplemental_dir`,
`area_file` → `fao_polygon_csv`, `log_path` → `log_file`.
...
External YAML aliases: `strict_pdf` → `strict_length_pdf`, `run_multiplier` → `run_multiplier_model`.
...
Trait source declarations. Each entry is one dataset. At minimum you need a
`study_metadata` entry pointing at your study workbook.
...
Reference-anchor selection settings passed to `set_reference_anchors()`.
...
| `distill_workers` | integer | `1` | Worker count for sigma-dropout sensitivity in `distill_traits()`. |
| `progress` | logical | `false` | Enable progress messages. |

### `alchemist.learner`

Controls the Super Learner trained inside `forge_distances()`. See the
[Super Learners vignette](super-learners.html) for guidance on method selection.

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `methods` | character vector | `["glm_elastic", "rf", "xgboost"]` | Base learner methods. See `list_learners()` for valid names. |
...
Trait selection, coherence, and kernel settings for the similarity matrix.
...
sub-section (`length`, `depth`, `frequency`) accepts `mode` and `weight`,
plus `gap` for `frequency`.
...
NMDS ordination settings, applied after `forge_distances()`.
...
| `nmds_args` | named list | — | Arguments forwarded to `vegan::metaMDS()`. |
| `envfit_args` | named list | — | Arguments forwarded to `vegan::envfit()`. |
...
Donors that fail an admissibility check are removed from the candidate pool
...
Constructor-style policy declarations. Use the `group` + `metric` + `branch`
form to generate named policy sets from the registry, or supply `active` as
an explicit list of policy name strings.
...
The conditional-uncertainty Super Learner. Trains a cross-fitted model to
predict expected absolute log-error, which is used to calibrate prediction
interval widths. See the [Super Learners vignette](super-learners.html).
...
`PolicyLearner`. See the [Super Learners vignette](super-learners.html).
...
| `select_terms` | `false` | Use `select = TRUE` in `mgcv::gam()` for additional shrinkage. |
...
| `variants` | `{}` | Named variant overrides (e.g. `shallow`, `deep`). Each variant becomes a distinct learner named `rpart_<variant>`. |
...
| `variants` | `{}` | Named variant overrides (e.g. `shallow`, `deep`). |
...
| `variants` | `{}` | Named variant overrides (e.g. `conservative`, `flexible`). |
...
Outer-loop validation and ablation settings for `build_sentinel()` /
`run_sentinel()`.
